...
- `FLASHVSR_STREAMING_LQ_MAX_BYTES`（默认 0 表示按分辨率自动估算）+ `FLASHVSR_STREAMING_PREFETCH_FRAMES` 控制纯内存 LQ 流式缓冲：后台线程按预读阈值（至少 8n+1 帧）填充，推理线程在每个 8 帧窗口结束后调用 `release_until` 即刻释放旧帧，全程不写 `*.memmap`。自动模式下缓冲区容量至少覆盖 `max(FLASHVSR_STREAMING_PREFETCH_FRAMES, 50)` 帧的 LQ 工作集；如需一次性把小视频载入 CPU，可把该上限调成更大的值，例如 `32GB` 或 `64GB`。
- 输出始终按 `FLASHVSR_CHUNKED_SAVE_CHUNK_SIZE` 帧拆分写入 `backend/storage/tmp/chunks_*`，异步落盘后使用 FFmpeg concat 合并；所有任务统一走分片写入与合并逻辑，方便在失败时基于分片恢复部分结果。
